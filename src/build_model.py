...
class ConstructCNN:
    ...
    def _convo_layers(self, ly_in: KerasTensor, filters: list) -> KerasTensor:
        ...
        cl3 = tkl.Conv1D(filters=filters[2], kernel_size=2, strides=1, activation=self.act_type, name='Convo1D-3')(ap2)
        ap3 = tkl.AveragePooling1D(pool_size=2, strides=1, name='AvgPooling1D-3')(cl3)
        cl4 = tkl.Conv1D(filters=filters[3], kernel_size=3, strides=1, activation=self.act_type, name='Convo1D-4')(ap3)
        ap4 = tkl.AveragePooling1D(pool_size=3, strides=1, name='AvgPooling1D-4')(cl4)
        cl5 = tkl.Conv1D(filters=filters[4], kernel_size=3, strides=1, activation=self.act_type, name='Convo1D-5')(ap4)
    ...
    def _dense_layers(self, ly_in: KerasTensor, units: list) -> KerasTensor:
        ...
        :return: Finally, this method returns the output of the last layer of the block.
        """
        dl_in = tkl.Dense(units=units[0], activation=self.act_type, name='Ann-in')(ly_in)
