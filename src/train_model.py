...
def fitting(model: Model, train_dataset: tf.Tensor, val_dataset: tf.Tensor, checkpoint_path: str) -> Model:
    """
    ...
    save_check_point = tf.keras.callbacks \
        .ModelCheckpoint(config['checkpoint_loc'] + '/' + checkpoint_path,
                         monitor=f'val_loss', save_best_only=True, mode='min',
    ...
    model.fit(train_dataset.batch(config['batch_size'] * gpu_count),
              validation_data=val_dataset.batch(config['batch_size'] * gpu_count), epochs=config['epochs'], verbose=2,
