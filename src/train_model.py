...
def fitting(model: Model, train_dataset: tf.Tensor, val_dataset: tf.Tensor) -> Model:
    """
    ...
    model.fit(train_dataset.batch(config['batch_size']), validation_data=val_dataset.batch(config['batch_size']),
              ...
              callbacks=[save_check_point, tensorboard_callback, time_stopping, early_stopping_callback])
    return model
